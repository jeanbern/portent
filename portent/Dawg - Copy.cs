...
namespace Portent
{
    public sealed unsafe class DawgSync : IDisposable
    {
        ...
        public IEnumerable<SuggestItem> Lookup(in string word, uint maxEdits)
        {
            if (maxEdits == 0)
            {
                ...
                _singleWordResult.Update(word, 0);
...
#pragma warning restore S1135 // Track uses of "TODO" tags
            ...
            *input = (char) 0;
            ...
            Search3Closure closureArg = new Search3Closure();
            closureArg._first = input;
            closureArg._toCache = toCache;
            nint maxPlus1 = maxPlusOne;
            closureArg._maxPlusOne = (uint) maxPlus1;
            closureArg._graph = _graph;
            closureArg._results = new List<SuggestItem>();
            closureArg._wordLength = wordLength;
            closureArg._builderDepth = 0;

            for (nint position = _rootFirstChild; position < _rootLastChild; ++position)
            {
                Search5(ref closureArg, position);
            }

            return closureArg._results;
        }

        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        [SuppressMessage("Major Code Smell", "S907:\"goto\" statement should not be used")]
        private static void Search5(ref Search3Closure closureArg, nint edge)
        {
            uint wordLength = closureArg._wordLength;
            nint maxPlus1 = closureArg._maxPlusOne;
            ...
            static void MatchCharacter(ref Search3Closure closure, nuint skip)
            {
                ...
                if (node < 0)
                {
                    ...
                    if (currentRowPreviousColumn < maxPlusOne && (uint) newDepth + maxPlusOne > closure._wordLength)
                    {
                        ...
                        closure._results.Add(new SuggestItem(stringResult, 0));
...
#pragma warning disable S3898 //Value types should implement "IEquatable<T>"
        private ref struct Search3Closure
        {
            ...
            [FieldOffset(0x78)]
            public List<SuggestItem> _results;
...
#pragma warning restore S1116 // Empty statements should be removed
        ...
        public DawgSync(Stream stream) : this(CompressedSparseRowPointerGraph.Read(stream))
        {
        }

        public DawgSync(CompressedSparseRowPointerGraph compressedSparseRows)
        {
        ...
        private readonly DawgGraph _graph;
