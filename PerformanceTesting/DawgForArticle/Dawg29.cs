...
{
    public sealed unsafe class Dawg29 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg29(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg29(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg29(PointerGraph graph)
        {
            _graph = graph;
