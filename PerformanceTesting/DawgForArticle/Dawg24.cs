...
{
    public sealed unsafe class Dawg24 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg24(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg24(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg24(PointerGraph graph)
        {
            _graph = graph;
