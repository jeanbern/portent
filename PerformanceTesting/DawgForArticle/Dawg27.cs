...
{
    public sealed unsafe class Dawg27 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg27(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg27(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg27(PointerGraph graph)
        {
            _graph = graph;
