...
{
    public sealed unsafe class Dawg22 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg22(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg22(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg22(PointerGraph graph)
        {
            _graph = graph;
