...
{
    public sealed unsafe class Dawg25 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg25(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg25(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg25(PointerGraph graph)
        {
            _graph = graph;
