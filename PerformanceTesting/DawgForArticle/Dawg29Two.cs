...
{
    public sealed unsafe class Dawg29Two : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg29Two(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg29Two(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg29Two(PointerGraph graph)
        {
            _graph = graph;
        ...
        private static void Recurse(int currentNode, long depth, long skip, ref ClosureVariable closure)
        {
            ...
            from &= ~from >> 63; // Equivalent to Max(from, 0)
            from += skip + 1;
            ...
            do
            {
                ...
                *builderPosition = currentCharacter;
                ...
                {
                    Recurse(nextNode, depth, skip + skip2, ref closure);
                }
