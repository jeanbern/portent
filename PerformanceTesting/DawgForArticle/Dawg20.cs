...
{
    public sealed unsafe class Dawg20 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg20(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg20(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg20(PointerGraph graph)
        {
            _graph = graph;
