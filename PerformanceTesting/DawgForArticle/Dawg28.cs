...
{
    public sealed unsafe class Dawg28 : IDawg
    {
        private readonly PointerGraph _graph;

        public Dawg28(Stream stream)
            : this(new CompressedSparseRowGraph(stream))
        { }

        public Dawg28(CompressedSparseRowGraph graph) : this(new PointerGraph(graph))
        { }

        public Dawg28(PointerGraph graph)
        {
            _graph = graph;
